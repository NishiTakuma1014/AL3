...
const char* sVersionString = "20250623";
...
const WORD kXInputButtons[] = {
    XINPUT_GAMEPAD_DPAD_UP,     XINPUT_GAMEPAD_DPAD_DOWN,     XINPUT_GAMEPAD_DPAD_LEFT,      XINPUT_GAMEPAD_DPAD_RIGHT, XINPUT_GAMEPAD_START, XINPUT_GAMEPAD_BACK, XINPUT_GAMEPAD_LEFT_THUMB,
    XINPUT_GAMEPAD_RIGHT_THUMB, XINPUT_GAMEPAD_LEFT_SHOULDER, XINPUT_GAMEPAD_RIGHT_SHOULDER, XINPUT_GAMEPAD_A,          XINPUT_GAMEPAD_B,     XINPUT_GAMEPAD_X,    XINPUT_GAMEPAD_Y,
};

bool IsPressXInputButton(const XINPUT_STATE& state, PadButton button) {
	// トリガーL2/R2特殊処理
	if (button == kPadButton14 && state.Gamepad.bLeftTrigger > 0x80) {
		return true;
	}
	if (button == kPadButton15 && state.Gamepad.bRightTrigger > 0x80) {
		return true;
	}
	// 普通のボタン
...
Sprite::BlendMode ToSpriteBlendMode(BlendMode blendMode) {
	...
	case kBlendModeExclusion:
		result = Sprite::BlendMode::kExclusion;
		break;
	default:
		assert(false);
		break;
...
struct RenderTargetSwitcher {
	RenderTargetSwitcher(BlendMode blendMode) : useSRGBRTV(blendMode != kBlendModeExclusion) {
		if (!useSRGBRTV) {
			DirectXCommon::GetInstance()->SetRenderTargets(false);
		}
	}
	...
	~RenderTargetSwitcher() {
		if (!useSRGBRTV) {
			DirectXCommon::GetInstance()->SetRenderTargets(true);
		}
	}
	bool useSRGBRTV = false;
};
...
private:
	...
	void BeginFrame();
	void EndFrame();
	Matrix4x4 Matrix4Orthographic(float viewLeft, float viewRight, float viewBottom, float viewTop, float nearZ, float farZ);
...
void NoviceSystem::CreateGraphicsPipelines() {
	...
	pipelineSetTriangles_[kBlendModeSubtract] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, kBlendModeSubtract);
	pipelineSetTriangles_[kBlendModeMultily] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, kBlendModeMultily);
	pipelineSetTriangles_[kBlendModeScreen] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, kBlendModeScreen);
	pipelineSetTriangles_[kBlendModeExclusion] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, kBlendModeExclusion);

	pipelineSetLines_[kBlendModeNone] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_LINE, kBlendModeNone);
	pipelineSetLines_[kBlendModeNormal] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_LINE, kBlendModeNormal);
	pipelineSetLines_[kBlendModeAdd] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_LINE, kBlendModeAdd);
	pipelineSetLines_[kBlendModeSubtract] = CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE_LINE, kBlendModeSubtract);
...
std::unique_ptr<NoviceSystem::PipelineSet> NoviceSystem::CreateGraphicsPipeline(D3D12_PRIMITIVE_TOPOLOGY_TYPE topologyType, BlendMode blendMode) {
	...
	gpipeline.RTVFormats[0] = (blendMode != kBlendModeExclusion) ? DXGI_FORMAT_R8G8B8A8_UNORM_SRGB : DXGI_FORMAT_R8G8B8A8_UNORM; // 0～255指定のRGBA
...
void NoviceSystem::CreateConstBuffer() {
	...
	// 平行投影による射影行列の生成
	Matrix4x4 mat = Matrix4Orthographic(0, float(dxCommon_->GetBackBufferWidth()), float(dxCommon_->GetBackBufferHeight()), 0, 0, 1);
...
std::unique_ptr<NoviceSystem::MeshForQuad> NoviceSystem::CreateMeshForQuad(UINT vertexCount, UINT indexCount) {
	HRESULT result;
...
void NoviceSystem::CreateSprites() {
	for (auto& sprite : sprites_) {
		sprite.reset(Sprite::Create(0, {0, 0}));
...
Microsoft::WRL::ComPtr<ID3D12Resource> NoviceSystem::CreateCommittedResource(UINT64 size) {
	...
	// リソース生成
	result = dxCommon_->GetDevice()->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&resource));
	assert(SUCCEEDED(result));
...
void NoviceSystem::DrawBox(int x, int y, int w, int h, float angle, unsigned int color) {
	...
	for (auto& vertex : vertices) {
		// 回転
		vertex.pos = {vertex.pos.x * cosf(angle) + vertex.pos.y * -sinf(angle), vertex.pos.x * sinf(angle) + vertex.pos.y * cosf(angle), vertex.pos.z};
		// 平行移動
	...
	// 描画コマンド
	commandList->DrawIndexedInstanced(kIndexCountBox, 1, static_cast<UINT>(indexIndex), static_cast<INT>(indexVertex), 0);
	// 使用カウント上昇
	indexBox_++;
}

void NoviceSystem::DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int color) {
	...
	// 頂点バッファへのデータ転送
	std::memcpy(&triangle_->vertMap[indexVertex], vertices.data(), sizeof(vertices[0]) * vertices.size());
...
void NoviceSystem::DrawTriangles(std::span<VertexPosColor> trianglePoints) {
	...
	//  頂点バッファへのデータ転送
	std::memcpy(&triangle_->vertMap[indexVertex], trianglePoints.data(), sizeof(trianglePoints[0]) * trianglePoints.size());
...
void NoviceSystem::DrawLines(std::span<VertexPosColor> linePoints) {
	...
	//  頂点バッファへのデータ転送
	std::memcpy(&line_->vertMap[indexVertex], linePoints.data(), sizeof(linePoints[0]) * linePoints.size());
...
void NoviceSystem::DrawSpriteRect(int destX, int destY, int srcX, int srcY, int srcW, int srcH, int textureHandle, float scaleX, float scaleY, float angle, unsigned int color) {
...
void NoviceSystem::DrawQuad(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int srcX, int srcY, int srcW, int srcH, int textureHandle, unsigned int color) {
	...
	// 頂点データ
	std::array vertices = {
	    Sprite::VertexPosUv{{static_cast<float>(x1), static_cast<float>(y1), 0.0f}, {uvLeft, uvTop}    },
	    Sprite::VertexPosUv{{static_cast<float>(x2), static_cast<float>(y2), 0.0f}, {uvRight, uvTop}   },
	    Sprite::VertexPosUv{{static_cast<float>(x3), static_cast<float>(y3), 0.0f}, {uvLeft, uvBottom} },
	    Sprite::VertexPosUv{{static_cast<float>(x4), static_cast<float>(y4), 0.0f}, {uvRight, uvBottom}},
	};
	...
	// 平行投影による射影行列の設定
	constBufferForQuads_[indexQuad_].address->mat = Matrix4Orthographic(0, float(dxCommon_->GetBackBufferWidth()), float(dxCommon_->GetBackBufferHeight()), 0, 0, 1);
	// 頂点バッファの設定
	commandList->IASetVertexBuffers(0, 1, &quad_->vbView);
	// インデックスバッファの設定
	commandList->IASetIndexBuffer(&quad_->ibView);
	// CBVをセット（ワールド行列）
	commandList->SetGraphicsRootConstantBufferView(0, constBufferForQuads_[indexQuad_].resource->GetGPUVirtualAddress());
	// シェーダリソースビューをセット
...
bool NoviceSystem::GetJoystickState(int stickNo, DIJOYSTATE2& out) { return input_->GetJoystickState(stickNo, out); }

bool NoviceSystem::GetJoystickStatePrevious(int stickNo, DIJOYSTATE2& out) { return input_->GetJoystickStatePrevious(stickNo, out); }

bool NoviceSystem::GetJoystickState(int stickNo, XINPUT_STATE& out) { return input_->GetJoystickState(stickNo, out); }
...
void NoviceSystem::SetAudioVolume(int playHandle, float volume) { audio_->SetVolume((uint32_t)playHandle, (std::max)(0.0f, volume)); }
...
void NoviceSystem::AllowWindowSizeChanged(bool allow) { winApp_->SetSizeChangeMode(allow ? WinApp::SizeChangeMode::kFixedAspect : WinApp::SizeChangeMode::kNone); }
...
Matrix4x4 NoviceSystem::Matrix4Orthographic(float viewLeft, float viewRight, float viewBottom, float viewTop, float nearZ, float farZ) {
...
std::unique_ptr<NoviceSystem> sNoviceSystem = nullptr;

} // namespace

void Novice::Initialize(const char* title, int width, int height, bool enableDebugLayer) {
	// 既に初期化済み
	assert(!sWinApp);
	assert(!sDxCommon);
	assert(!sInput);
	...
	// ゲームウィンドウの作成
	std::string titleWithVersion = std::string(title) + " with Novice " + sVersionString;
	auto&& titleString = ConvertString(titleWithVersion);
	sWinApp = WinApp::GetInstance();
	sWinApp->CreateGameWindow(titleString, WS_OVERLAPPEDWINDOW & ~(WS_MAXIMIZEBOX | WS_THICKFRAME), width, height);
	sWinApp->SetSizeChangeMode(WinApp::SizeChangeMode::kNone);

	// DirectX初期化処理
	sDxCommon = DirectXCommon::GetInstance();
	sDxCommon->Initialize(width, height, enableDebugLayer);
	...
	// テクスチャマネージャの初期化
	TextureManager::GetInstance()->Initialize(GetResourceRootChar());
	...
	// 入門用システム初期化
	sNoviceSystem = std::make_unique<NoviceSystem>();
...
void Novice::DrawBox(int x, int y, int w, int h, float angle, unsigned int color, FillMode fillMode) {
	if (fillMode == kFillModeSolid) {
		...
		for (auto& vertex : vertices) {
			// 回転
			vertex = {vertex.x * cosf(angle) + vertex.y * -sinf(angle), vertex.x * sinf(angle) + vertex.y * cosf(angle)};
			// 平行移動
			vertex.x += static_cast<float>(x);
			vertex.y += static_cast<float>(y);
		}
		// 左上から右上
		sNoviceSystem->DrawLine(int(vertices[1].x), int(vertices[1].y), int(vertices[2].x), int(vertices[2].y), color);
		// 左上から左下
		sNoviceSystem->DrawLine(int(vertices[1].x), int(vertices[1].y), int(vertices[0].x), int(vertices[0].y), color);
		// 右上から右下
		sNoviceSystem->DrawLine(int(vertices[2].x), int(vertices[2].y), int(vertices[3].x), int(vertices[3].y), color);
		// 左下から右下
		sNoviceSystem->DrawLine(int(vertices[0].x), int(vertices[0].y), int(vertices[3].x), int(vertices[3].y), color);
	}
}

void Novice::DrawTriangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int color, FillMode fillMode) {
	if (fillMode == kFillModeSolid) {
...
void Novice::DrawLine(int x1, int y1, int x2, int y2, unsigned int color) { sNoviceSystem->DrawLine(x1, y1, x2, y2, color); }

void Novice::DrawEllipse(int x, int y, int radiusX, int radiusY, float angle, unsigned int color, FillMode fillMode) {
	constexpr size_t kNumDivision = 48;                                              // 分割数
	constexpr float kRadianPerDivision = 3.1415926535f * 2.0f / float(kNumDivision); // 分割1単位の角度
	...
	if (fillMode == kFillModeSolid) {
		...
		for (size_t index = 0; index < kNumDivision; ++index) {
			...
			if (index == 0) {
				...
				points[index * 3 + 1].pos = {baseX * angleCos - baseY * angleSin + x, baseY * angleCos + baseX * angleSin + y, 0};
			} else {
				points[index * 3 + 1].pos = points[(index - 1) * 3 + 2].pos;
			}

			points[index * 3 + 1].color = colorf;
			points[index * 3 + 2].pos = {nextBaseX * angleCos - nextBaseY * angleSin + x, nextBaseY * angleCos + nextBaseX * angleSin + y, 0};
		...
		for (size_t index = 0; index < kNumDivision; ++index) {
			...
			if (index == 0) {
				float baseX = std::cos(kRadianPerDivision * index) * radiusX;
				float baseY = std::sin(kRadianPerDivision * index) * radiusY;
				points[index * 2].pos = {baseX * angleCos - baseY * angleSin + x, baseY * angleCos + baseX * angleSin + y, 0};
			} else {
				points[index * 2].pos = points[(index - 1) * 2 + 1].pos;
			}

			points[index * 2].color = colorf;
			points[index * 2 + 1].pos = {nextBaseX * angleCos - nextBaseY * angleSin + x, nextBaseY * angleCos + nextBaseX * angleSin + y, 0};
...
int Novice::LoadTexture(const char* fileName) { return static_cast<int>(TextureManager::Load(fileName)); }

void Novice::UnloadTexture(int textureHandle) { TextureManager::Unload(textureHandle); }

void Novice::DrawSprite(int x, int y, int textureHandle, float scaleX, float scaleY, float angle, unsigned int color) {
	sNoviceSystem->DrawSpriteRect(x, y, -1, -1, -1, -1, textureHandle, scaleX, scaleY, angle, color);
}

void Novice::DrawSpriteRect(int destX, int destY, int srcX, int srcY, int srcW, int srcH, int textureHandle, float scaleX, float scaleY, float angle, unsigned int color) {
	sNoviceSystem->DrawSpriteRect(destX, destY, srcX, srcY, srcW, srcH, textureHandle, scaleX, scaleY, angle, color);
}

void Novice::DrawQuad(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int srcX, int srcY, int srcW, int srcH, int textureHandle, unsigned int color) {
	sNoviceSystem->DrawQuad(x1, y1, x2, y2, x3, y3, x4, y4, srcX, srcY, srcW, srcH, textureHandle, color);
}

int Novice::LoadAudio(const char* fileName) { return sNoviceSystem->LoadAudio(fileName); }

int Novice::PlayAudio(int soundHandle, int loopFlag, float volume) { return sNoviceSystem->PlayAudio(soundHandle, loopFlag, volume); }
...
void Novice::SetAudioVolume(int playHandle, float volume) { sNoviceSystem->SetAudioVolume(playHandle, volume); }

int Novice::IsPlayingAudio(int playHandle) { return sNoviceSystem->IsPlayingAudio(playHandle) ? 1 : 0; }
...
int Novice::IsPressMouse(int buttonNumber) { return sNoviceSystem->IsPressMouse(buttonNumber) ? 1 : 0; }

int Novice::IsTriggerMouse(int buttonNumber) { return sNoviceSystem->IsTriggerMouse(buttonNumber) ? 1 : 0; }
...
int Novice::GetMousePosition(int* positionX, int* positionY) {
	...
	if (positionX && positionY) {
		...
		float normalizedX = mousePosition.x / static_cast<float>(clientRect.right - clientRect.left);
		float normalizedY = mousePosition.y / static_cast<float>(clientRect.bottom - clientRect.top);

		*positionX = static_cast<int>(normalizedX * sNoviceSystem->dxCommon_->GetBackBufferWidth());
		*positionY = static_cast<int>(normalizedY * sNoviceSystem->dxCommon_->GetBackBufferHeight());
		return 1;
	}
	return 0;
}
...
void Novice::SetIcon(const char* fileName) {
	...
	HICON smallIconHandle = static_cast<HICON>(LoadImageA(GetModuleHandle(NULL), iconPath.c_str(), IMAGE_ICON, 16, 16, LR_LOADFROMFILE));
	HICON largeIconHandle = static_cast<HICON>(LoadImageA(GetModuleHandle(NULL), iconPath.c_str(), IMAGE_ICON, 32, 32, LR_LOADFROMFILE));
	SendMessage(sWinApp->GetHwnd(), WM_SETICON, ICON_SMALL, reinterpret_cast<LPARAM>(smallIconHandle));
	SendMessage(sWinApp->GetHwnd(), WM_SETICON, ICON_BIG, reinterpret_cast<LPARAM>(largeIconHandle));
}

void Novice::SetWindowMode(WindowMode windowMode) { sNoviceSystem->SetFullscreen(windowMode == kFullscreen); }
...
int Novice::IsTriggerButton(int stickNo, PadButton button) {
	...
	if (sNoviceSystem->GetJoystickState(stickNo, stateDInput) && sNoviceSystem->GetJoystickStatePrevious(stickNo, statePreviousDInput)) {
		return ((stateDInput.rgbButtons[button] & 0x80) && !(statePreviousDInput.rgbButtons[button] & 0x80)) ? 1 : 0;
	} else if (sNoviceSystem->GetJoystickState(stickNo, stateXInput) && sNoviceSystem->GetJoystickStatePrevious(stickNo, statePreviousXInput)) {
		return IsPressXInputButton(stateXInput, button) && !IsPressXInputButton(statePreviousXInput, button) ? 1 : 0;
	}
